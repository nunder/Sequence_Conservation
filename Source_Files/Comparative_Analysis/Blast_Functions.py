...
def run_blastp(blast_dir, query_file, blast_db_name, outfile, e_value = 1e-10): 
    w_d = os.getcwd()
    os.chdir("F:/")
    subprocess.run('cd '+blast_dir+' & blastp -query '+blast_dir+'/'+query_file+' -db '+blast_db_name +' -out ' + outfile + ' -evalue '+str(e_value) +' -seg no -outfmt  "10 qaccver saccver qlen slen pident length mismatch gapopen qstart qend sstart send evalue bitscore" -num_threads 16', shell=True, capture_output = True)
    os.chdir(w_d)
    
def run_blastn(blast_dir, query_file, blast_db_name, outfile, e_value = 1e-10): 
    w_d = os.getcwd()
    os.chdir("F:/")
    subprocess.run('cd '+blast_dir+' & blastn -query '+blast_dir+'/'+query_file+' -db '+blast_db_name +' -out ' + outfile + ' -evalue ' +str(e_value) +' -outfmt  "10 qaccver saccver qlen slen pident length mismatch gapopen qstart qend sstart send evalue bitscore" -num_threads 16', shell=True, capture_output = True)
    os.chdir(w_d)

def run_tblastx(blast_dir, query_file, blast_db_name, outfile, e_value = 1e-10): 
    w_d = os.getcwd()
    os.chdir("F:/")
    subprocess.run('cd '+blast_dir+' & tblastx -query '+blast_dir+'/'+query_file+' -db '+blast_db_name +' -out ' + outfile + ' -evalue ' +str(e_value) +' -outfmt  "10 qaccver saccver qlen slen pident length mismatch gapopen qstart qend sstart send evalue bitscore" -num_threads 16', shell=True, capture_output = True)
    os.chdir(w_d)
    
def run_tblastn(blast_dir, query_file, blast_db_name, outfile, e_value = 1e-10): 
    w_d = os.getcwd()
    os.chdir("F:/")
    subprocess.run('cd '+blast_dir+' & tblastn -query '+blast_dir+'/'+query_file+' -db '+blast_db_name +' -out ' + outfile + ' -evalue ' +str(e_value) +' -outfmt  "10 qaccver saccver qlen slen pident length mismatch gapopen qstart qend sstart send evalue bitscore" -num_threads 16', shell=True, capture_output = True)
    os.chdir(w_d)


def process_blast_output(infile_loc, names_dict, top_hit_only = False, outfile_loc = 'None'):
    ...
    if not(outfile_loc == 'None'):
        with open(outfile_loc, 'wb') as f:
            pickle.dump(blast_results, f)
    return blast_results

def keep_reciprocal_best_hits(query_df, reverse_query_df, outfile_loc ='None'):
    temp_1_dict = {}
    temp_2_dict = {}
