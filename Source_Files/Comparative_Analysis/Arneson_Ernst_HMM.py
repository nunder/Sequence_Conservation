...
class Alignment_HMM:
    ...
    def __init__(self, num_symbols, num_states, alignment_dir, master_species_id, species_order = []):
        ...
        self.alignment_ids = [(i.split('.')[0]) for i in alignment_ids]
        self.alignment_dict = {}
    ...
    def alignment_hmm_log_likelihood(self, params, num_subsets, subset_num, min_length, all_species = True, comparison_species = ''):
        total_probability = 0
        ...
        for group_id in ids:
            ...
            if len(align_list[0]) < min_length:
                continue
            observation_probabilities = self.calculate_observation_probs(mutation_probabilities, align_list, alignment)
    ...
    def EM_update_parameters(self, num_subsets, subset_num, min_length, mutation_probabilities, transition_probabilities, all_species, comparison_species):
        ...
        for group_id in ids:
            ...
            if len(align_list[0]) < min_length:
                continue
            match_probs =  self.calculate_match_probs(align_list, alignment)    
            observation_probabilities = self.calculate_observation_probs(mutation_probabilities, align_list, alignment, True, '')
    ...
    def EM_update(self, num_subsets, params, min_length, all_species = True, comparison_species = ''):
        num_comparison_sequences = 10
