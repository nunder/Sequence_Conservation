...
class Master_Alignment_HMM:
        ...
        for i in range(num_observations):
            for s in range(self.num_states):
                ...
                for k in range(num_sequences):
                    temp = temp * (mutation_probabilities[s]*observations[k][0][i] + (1-mutation_probabilities[s])*observations[k][1][i])
    ...
    def EM_update(self, num_subsets, params, offset, min_length):
        ...
        for iternum in tqdm(range(300)):
            ...
            #parallel_output = [self.EM_update_parameters(1, 1, offset, min_length, mutation_probabilities, transition_probabilities) ]
